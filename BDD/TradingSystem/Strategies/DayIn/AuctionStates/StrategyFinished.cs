namespace TradingSystem.Strategies.DayIn.AuctionStates;

public class StrategyFinished : IAuctionState
{
    public StrategyFinished(StrategyContext context) => Context = context;

    public IAuctionState Start() => this;

    public StrategyContext Context { get; }
    public IAuctionState OnTradingSessionChanged(TradingSessionType newSessionType) => this;

    public void OnParentModificationRequested(TsOrder newParent)
    {
    }

    public IAuctionState OnParentCancellationRequested() => this;

    public void OnChildRejected()
    {
    }

    public IAuctionState OnChildFilled(TsOrder child) => this;
}