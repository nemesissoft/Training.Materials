...
public class DayInAuction : IStrategy
{
    ...
    public DayInAuction(
        ...
        IOrderManager orderManager) => _auctionState = AuctionStateFactory.Created(
            new StrategyContext(parentOrder, sessionType, new OrderManagerWrapper(orderManager), StrategyFinishedProcessing));

    public void Start() => _auctionState = _auctionState.Start();

    public void onNewMarketData(MarketData marketData)
    {
    }

    public void onTradingSessionChanged(TradingSessionType session) => _auctionState = _auctionState.OnTradingSessionChanged(session);
